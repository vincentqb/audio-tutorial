...
def process_waveform(waveform):
    print("process waveform")
    return mfcc(waveform)[0, ...]
...
def datasets():
    ...
    training = FILTERED_SPEECHCOMMANDS("training", root, download=download)
    # training = MemoryCache(training)
    validation = FILTERED_SPEECHCOMMANDS("validation", root, download=download)
    # validation = MemoryCache(validation)
    testing = FILTERED_SPEECHCOMMANDS("testing", root, download=download)
...
def collate_fn(batch):
    ...
    tensors = [b[0].transpose(0, -1) for b in batch if b]

    target_lengths = [len(t) for t in targets]
    input_lengths = [len(t) for t in tensors]
...
training_tensors, training_targets, _, _ = collate_fn(training)
validation_tensors, validation_targets, _, _ = collate_fn(validation)

training_tensors = training_tensors.to(device, non_blocking=non_blocking)
validation_tensors = validation_tensors.to(device, non_blocking=non_blocking)

training_tensors = process_waveform(training_tensors)
validation_tensors = process_waveform(validation_tensors)

training_targets = training_targets.to(device, non_blocking=non_blocking)
validation_targets = validation_targets.to(device, non_blocking=non_blocking)

model = Wav2Letter(n_mfcc, vocab_size)

# hidden_size = 128
# model = nn.GRU(num_features, hidden_size, num_layers=3, batch_first=True, bidirectional=True)
# model = nn.LSTM(num_features, hidden_size, num_layers=3, batch_first=True, bidirectional=True)
...
for epoch in range(max_epoch):
    ...
    sum_loss = 0.
    length = len(training_tensors)
    for i0 in tqdm(range(0, length, batch_size)):
        i1 = min(i0 + batch_size, length)
        
        inputs = training_tensors[i0:i1, ...]
        targets = training_targets[i0:i1, ...]
